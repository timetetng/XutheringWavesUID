...
async def draw_tower_challenge_img(
    ev: Event, period: Optional[int] = None
) -> Union[bytes, str]:
    """绘制深塔信息"""
    try:
        # 确定期数
        ...
        # 绘制标题
        draw_text_with_shadow(
            draw, "深塔", width // 2, 50, waves_font_32, "white", anchor="mm"
        )

        # 绘制期数标签（在标题下方）
        draw_text_with_shadow(
            draw, f"第{period}期", 50, 95, waves_font_20, (180, 180, 180), anchor="lm"
        )
        ...
        for i, (area_name, floor_data) in enumerate(sections):
            ...
            _draw_floor_section(
                card_img, (40, current_y), area_name, floor_data, width - 80, section_h
            )
...
async def draw_slash_challenge_img(
    ev: Event, period: Optional[int] = None
) -> Union[bytes, str]:
    """绘制海墟信息"""
    try:
        ...
        # 绘制标题
        draw_text_with_shadow(
            draw, f"海墟 第{period}期", width // 2, 50, waves_font_32, "white", anchor="mm"
        )
        
        draw_text_with_shadow(
            draw, f"无尽 - {title}", width // 2, 90, waves_font_24, (255, 200, 100), anchor="mm"
        )

        # 绘制海域特性(Desc)
        current_y = desc_start_y
        draw_text_with_shadow(
            draw, "【海域特性】", 50, current_y, waves_font_20, (255, 200, 100), anchor="lm"
        )
        current_y += 30
        ...
        if buff_data:
            current_y += 10
            draw_text_with_shadow(
                draw, "【本期信物】", 50, current_y, waves_font_20, (255, 215, 0), anchor="lm"
            )
            current_y += 35
        ...
        for i, floor_data in enumerate(floor_list):
            # 绘制分割标题
            draw_text_with_shadow(
                draw, f"【半场 {i+1}】", 50, current_y, waves_font_20, (100, 200, 255), anchor="lm"
            )
            current_y += 30
            ...
            if monsters:
                draw_text_with_shadow(
                    draw, "敌人配置", 65, current_y, waves_font_18, (255, 150, 150), anchor="lm"
                )
                current_y += 35
                ...
                for monster_info in list(monsters.values())[:8]:
                    ...
                    draw.text(
                        (x_pos + card_w - w_elem - 10, current_y + 10),
                        element_name,
                        color,
                        waves_font_18,
                        "lm"
...
def _calculate_section_height(area_name: str, floor_data: Dict[str, Any], width: int) -> int:
    ...
    if buffs: section_height += 30
    if monsters: section_height += 45
    
    if section_height < 180: section_height = 180
...
def _draw_floor_section(
    ...
    if monsters:
        ...
        for monster_info in list(monsters.values())[:6]:
            ...
            # 绘制小背景
            draw.rounded_rectangle(
                (curr_x, current_y - 15, curr_x + card_w, current_y + 15),
                radius=4,
                fill=(0, 0, 0, 100),
                outline=color
            )
            ...
            draw.text(
                (curr_x + card_w - w_elem - 10, current_y),
                element_name,
                color,
                waves_font_14,
                "lm"
